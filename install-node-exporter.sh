...
# 取得最新版本號
LATEST_VERSION=$(curl -fsSL https://api.github.com/repos/prometheus/node_exporter/releases/latest \
    | grep '"tag_name"' | head -1 | cut -d '"' -f4 | sed 's/^v//')

if [ -z "$LATEST_VERSION" ]; then
    echo "無法取得最新版本號（可能是 GitHub API rate limit）" >&2
    exit 1
fi

echo "Installing node_exporter v${LATEST_VERSION} (${ARCH})"

# 下載並解壓到 /usr/local/bin
...
install -m 0755 -o node_exporter -g node_exporter \
    "${TMPDIR}/node_exporter-${LATEST_VERSION}.linux-${ARCH}/node_exporter" \
    /usr/local/bin/node_exporter

# 預設選項檔（可選，方便日後加 --collector.* 參數）
