...
if [[ "${CLOUD}" == "unknown" ]]; then
  err "[錯誤] 無法辨識的平台。本腳本僅適用於 Aliyun ECS & AWS EC2 的 Rocky Linux。"
  exit 1
fi
...
SSHD_CONF="/etc/ssh/sshd_config.d/99-init.conf"
cat > "${SSHD_CONF}" <<'EOF'
